...
# Single NAT gateway (not one per AZ) to keep cost down for this draft.
# Trade-off: a single point of failure for private-subnet egress; add a
# second NAT (one per AZ) later if that risk needs to be eliminated.
